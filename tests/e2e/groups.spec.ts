...
test.describe('Groups Page', () => {
  test('should load groups listing page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /developer groups/i })).toBeVisible();
  });
  ...
  test('should display call to action for new meetups', async ({ page }) => {
    ...
    await expect(page.getByText(/if you'd like to start a meetup/i)).toBeVisible();
    await expect(page.getByRole('link', { name: /syracuse\.io slack/i })).toBeVisible();
  });
});
